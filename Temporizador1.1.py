...
class Temporizador1(Thread):
    def __init__(self, hora, delay):
        ...
        super(Temporizador1, self).__init__()
        self._estado = True
        self.hora = hora
        self.delay = delay
    ...
    def run(self):
        ...
        while self._estado:
            ...
            if datetime.now() > hora and self.si_hice_la_comprobacion() == False:
                ...
                hora += timedelta(days=1)
                #print('Proxima ejecucion programada el {0} a las {1}'.format(hora.date(), hora.time()))
                self.actualizar_achivo_de_comprobacion(datetime.now().day)
                #self.stop()
    ...
    def actualizar_achivo_de_comprobacion(self, dia_de_comprobacion):
        if dia_de_comprobacion is None:
            print('dia_de_comprobacion: NONE\nNONE\nNONE\nNONE\nNONE\nNONE\nNONE\nNONE')
        try:
